...
type Props = {
  ...
  onSignup: (payload: { fullName: string; email: string; password: string; role: UserRole }) => Promise<{ ok: boolean; error?: string }>;
};
...
export function LoginScreen({ onLogin, onSignup }: Props) {
  ...
  async function handleSubmit() {
    ...
    const result = mode === "login"
      ...
      : await onSignup({ fullName, email, password, role });
